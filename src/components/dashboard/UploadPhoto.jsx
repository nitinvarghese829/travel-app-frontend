    ...
    return (
        <>
            <div className={'flex gap-2'}>
                <input className={'input'} type={'text'} value={photoLink} onChange={(e) => setPhotoLink(e.target.value)} />
                <button className={'bg-primary px-4 text-wrap whitespace-nowrap text-white rounded-full'} onClick={addPhoto}>Add Photo</button>
            </div>
            ...
            <div className={'flex gap-2'}>
                {addedPhotos && addedPhotos.length > 0 && addedPhotos.map((link, index) => (
                    <div className={'relative'}>
                        <div>
                            <img className={'rounded-2xl object-fill w-64 h-64'} src={`${BASE_URL}/uploads/`+link} key={index} />
                        </div>
